...
sys.path.append(os.path.join(workDir, 'platform', 'edk2','bios'))
...
def check_sonic_version(show_version_cmd, show_boot_cmd, uname_cmd):
    ...
    print('Exp version is ',exp_version)
...
def sonic_booting_info_check(coreboot_release_date):
    ...
    output=device.executeCmd('sudo reboot')
    time.sleep(100)
    exit_the_shell()
...
def check_software_version():
    ...
    if expect_sonic_version in sonic_fw_version:
        ...
        raise RuntimeError(f'Expected sonic version \'{expect_sonic_version}\' not present in dut')
    if expect_bios_version in bios_fw_version:
        log.success(f'Expeced bios version\'{expect_bios_version}\' present in the dut')
    else:
        raise RuntimeError(f'Expetced bios version\'{expect_bios_version}\' not present in dut')
...
def COMe_CPLD_register_access():
    #Powercycle device
    device.sendCmd('echo 0x4449454a > /sys/devices/gfpga-platform/board_powercycle')
    time.sleep(200)
    exit_the_shell()
    ...
    condition1 = "0x55" not in cpld_val_1 or "0x55" not in cat_come_val #Should be False
    if condition1:
...
def tlv_eeprom_from_diag_cmd():
    device.executeCmd('cd /usr/local/cls_diag/bin')
    diag_output1 = run_command('./cel-eeprom-test -r -d all -t tlv')
    diag_output2 = run_command('./cel-eeprom-test --dump -d all -t tlv')
...
#    diagos_login_check()
    output = device.executeCmd('cat /sys/devices/gfpga-platform/osfp_pd_l')
...
def check_SFPplus_port_eeprom_info():
    ...
    if 'error' in output3:
        raise RuntimeError('physical port 33,34 is not connected to sfp+ dac cable.\nsfp+ module is required for eeprom info')
    else:
...
def switch_adm1266_to_fpga_pmbus():
    run_command('modprobe -r adm1266', prompt=None)
    run_command('echo 0x44 > /sys/bus/i2c/devices/i2c-27/delete_device', prompt=None)
    run_command('echo 1 > /sys/devices/gfpga-platform/seq_mux_sel', prompt=None)
    run_command('sleep 1', prompt=None)
    run_command('echo adm1266 0x44 > /sys/bus/i2c/devices/i2c-10001/new_device', prompt=None)
    run_command('sleep 1',prompt=None)
...
def Fan_Speed_Control():
    ...
    #output = device.sendCmd('echo 0x4449454a > /sys/devices/gfpga-platform/board_powercycle', 'Shell>', timeout=220)
    #exit_the_shell()
    sleep(3)
    ...
    for lines in output.splitlines():
        if lines.startswith('   Main PID'):
            ...
            if pid:
                ...
                output = device.executeCmd('kill -9 '+ pid)
                time.sleep(100)
                exit_the_shell()
            else:
...
def i2c_stress_test():
    ...
    out10 = device.executeCmd('cat /sys/devices/platform/mmcpld/fantray.*.auto/hwmon/hwmon*/name')
...
def warm_boot_reset_stress_test():
    ...
    output = run_command('./'+filename, prompt='Shell>', timeout = '700')
    exit_the_shell()
...
def WDT_reset_stress_test():
    ...
    device.sendCmd('rm Check_Bus.txt count.log function_check.log')
    device.sendCmd('cd admin')
    check = run_command('cat '+filename)

    content = 'reset.sh'
    if re.search(content, check):
        output = run_command('./'+filename,  prompt='Shell>', timeout = '360')
        exit_the_shell()
        sleep(10)
    else:
...
def IdentifyCorebootImage():
    ...
    for item in output4.splitlines():
        if not re.search('16+0',item):
            ...
            break
    output = device.sendCmd('reboot', 'Shell>', timeout=450)
    exit_the_shell()
    sleep(10)
...
def upgrade_SONiC_Via_Current_Version():
    scp(filename2)
    sleep(30)
    device.sendMsg("sonic-installer install sonic-broadcom-pb22.bin \n")
    device.read_until_regexp("New image will be installed.*")
    device.sendMsg("y \r\n")
    output=device.read_until_regexp(".*Done",timeout=200)
    device.sendMsg("show boot \n")
...
def checkSdb():
    ...
    if re.search('Created a new DOS disklabel',c2):
        log.success('Operation successful')
    else:
        log.fail('Operation fail')
    c3=device.sendCmd('mkfs /dev/sdb1 ')
    device.sendMsg('y \n')
    ...
    print('The vlaue of c4 is ',c4)


@logThis
def exit_the_shell():
    device = DeviceMgr.getDevice()
    out=device.read_until_regexp('Shell>',timeout=120)
...
@logThis
def check_powercycle_device(c,a,b):
    powercycle_pdu1(c,a,b)

@logThis
def login_check():
    device = DeviceMgr.getDevice()
    device.read_until_regexp('command-line',timeout=100)
    device.sendCmd('\x1b[B')
    device.sendCmd('/r')
    time.sleep(10)
    device.sendCmd('/r')
    exit_the_shell()
