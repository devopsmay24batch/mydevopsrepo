...
def upgradeCpld():
    ...
    cmd=run_command("./cel-upgrade-test --update --dev 2 --file ../tools/firmware/COMe_CPLD_V18_20220913.vme",prompt='root@sonic')
...
def checkList(mod):
    ...
    if (mod =='all' or mod == 'i2c' or mod == 'pci' or mod == 'psu') and c100 == 'mtp':
        pattern = "var."+mod+"_list_mtp"
   
    print('The value of c100',c100)
    print('The list to be used is ',pattern)
    CommonKeywords.should_match_ordered_regexp_list(cmd1,eval(pattern))
...
def checkTest(mod):
    if mod == 'pci':
        cmd1= run_command("./cel-"+mod+"-test --all --file ../configs/pcis_edk2.yaml",prompt ='root@sonic')
    else:
...
def checkDiagVersion(flag):
    ...
    compare_version = brixia_diag_version if flag == "new" else brixia_old_diag_version
...
def fetch_Image_From_Server(image):
    ...
    output = run_command("curl -O http://10.208.29.3:8080/EDKII/" + image, prompt="root@sonic:.*", timeout=60)
...
def install_Diag_Package(diag_image):
    ...
    if c100 == 'brixia':
        ...
        run_command("./install -p capitaine", prompt="root@sonic:.*", timeout=15)
...
def check_Basic_Diag_Functions(flag=''):
    ...
    if c100 == 'mtp':
        print('Checking for mtp:')
        val1=structure_ptn_mtp
    else:
        print('Checking for brixia:')
        val1=structure_ptn
    for each in val1:
...
def change_Bios_version(cmd,device):
...
def checkLspci():
    ...
    CommonKeywords.should_match_ordered_regexp_list(cmd1,val1)
...
def checkPciFile():
    if c100 != 'mtp':
        ...
        log.success('PCie file operation skipped for mtp')
...
def checkCpldOperations():
    global d1_value
    val = ['Read value: 0x18','cpld test : Passed']
...
@logThis
def checkMajorMinor():
    c1= run_command("cat /sys/devices/gfpga-platform/majorrevision")
    c2= run_command("cat /sys/devices/gfpga-platform/minorrevision")
    c3= run_command("cat /sys/devices/gfpga-platform/scratch")
...
@logThis
def checkI2cdetect(device):
    run_command('sudo -s')
    devicename = os.environ.get("deviceName", "")
    device = DeviceMgr.getDevice()
    c2= DeviceMgr.getDevice(devicename).get('variant')
    c1= run_command("i2cdetect -l",prompt ='root@sonic')
    if c2 == 'mtp':
        CommonKeywords.should_match_ordered_regexp_list(c1,var.i2cdetect_list_mtp)
    else:
...
def checkTh4Operations():
    ...
    c6=run_command('./cel-detect-th4-test --list --file ../configs/detect_th4_b0.yaml')
    ...
    c10=run_command('./cel-detect-th4-test -d 3 -f ../configs/detect_th4_b0.yaml')
    c11=run_command('./cel-detect-th4-test --all -f ../configs/detect_th4_b0.yaml')
    ...
    c13=run_command('./cel-detect-th4-test --all --file ../configs/detect_th4.yaml')
...
def checkTempList():
    if c100!='mtp':
        c1=run_command("./cel-temp-test --list  --file ../configs/second_source/temp_flex.yaml")
        c2=run_command("./cel-temp-test -l  --file ../configs/second_source/temp_flex.yaml")
        val1=temp_list
    else:
        c1=run_command("./cel-temp-test --list  --file ../configs/temp.yaml")
        c2=run_command("./cel-temp-test -l  --file ../configs/temp.yaml")
        val1=temp_list_mtp
    CommonKeywords.should_match_ordered_regexp_list(c1,val1) 
    CommonKeywords.should_match_ordered_regexp_list(c2,val1)
...
def checkTempTest(mod):
    ...
    if c100!='mtp':
        ...
        val1=adapter_list_mtp

    for each in val1:
        if each not in output:
            print('Not matching pattern: ',each)
            raise Exception("Error in sensor list.\nConcerned pattern: "+each)
    if c100!='mtp':
        c11=run_command(' ./cel-temp-test --all --file ../configs/second_source/temp_flex.yaml')
        if re.search('Result:Temp test all --> FAILED <<<',c11):
            log.success('Test Failed as expected')
        else:
            log.fail('Test passed but expected fail')

        c12=run_command('./cel-temp-test --all --file ../configs/second_source/temp_astersyn.yaml')
        if re.search('Result:Temp test all --> FAILED <<<',c11):
            log.success('Test Failed as expected')
        else:
            log.fail('Test passed but expected fail')
    else:
        log.success('No secondary source for MtP')
...
def checkBaseboardList():
    if c100 =='mtp':
        log.success('Baseboard test skipped for MTP')
    else:
...
@logThis
def checkDowngradeUpgrade(val,device):
    ...
    c1=run_command('./cel-upgrade-test --update --dev 3 --file ../tools/firmware/'+filename)
    ...
    powercycle_device(device)   ;#Powercycle after operation
    verifyDiagToolPath()
...
def checkAllTests():
    ...
    for x in cmd_lst:
        #cmd1=run_command(x,prompt='#',timeout=10)
        device.sendCmd(x)

        time.sleep(3)
    run_command('docker exec -ti pmon supervisorctl stop thermalctld',prompt = None ,timeout=10)
    run_command('cp /home/all.yaml /usr/local/cls_diag/configs/.',prompt ='#') 
    run_command('ls -la log/',prompt ='#')
    if c100 == 'mtp':
        c1=run_command('./cel-all-test --all',prompt ='root@sonic',timeout=300)
        device.sendMsg('reset \n')
        patt = ['Failed.*0.*Passed:10.*Total.*10']
    else:
...
def checkStorageStress():
    ...
    if c100 == 'mtp':
        cmd1=device.executeCmd("./SSD_test.sh 300 test_SSD.log",timeout= 600)
...
@logThis
def checkRtcFile():
    c2=run_command('./cel-rtc-test --read',prompt =None)
    c1=run_command('./cel-rtc-test -l --f /home/rtc.yaml',prompt =None)
    CommonKeywords.should_match_ordered_regexp_list(c1,var.rtc_list)
    c3=run_command('hwclock',prompt =None)
    c2=re.search(r'[0-9]{4}\-[0-9]{2}\-[0-9]{2}',c2).group()
    ...
    c5=run_command('./cel-rtc-test -w -D "20300430 235959"',prompt =None)
    c6=re.search(r'[0-9]{4}\-[0-9]{2}\-[0-9]{2}',c5).group()
    c3=run_command('./cel-rtc-test -r',prompt =None)
    c4=re.search(r'[0-9]{4}\-[0-9]{2}\-[0-9]{2}',c3).group()
    ...
    c7=run_command('./cel-rtc-test --write --data "20211231 235959" --file /home/rtc.yaml',prompt =None)
    ...
    if c8 == c4:
        ...
        log.fail('Values are incorrect')
    c9= run_command('./cel-rtc-test -r',prompt =None)
...
def checkCpuFileOperation():
    run_command('cp /usr/local/cls_diag/configs/cpu.yaml  /home/')
    run_command('cd /home',prompt='root@sonic')
    time.sleep(2)
    run_command('sed -i \'s/cpu_cores:  16/cpu_cores:  8/g\' cpu.yaml')
    run_command('cd /usr/local/cls_diag/bin',prompt='root@sonic')
    run_command('./cel-cpu-test -l --file /home/cpu.yaml')
    c1=run_command('./cel-cpu-test --all -f /home/cpu.yaml')
...
def Checki2cdevice(detectdevice_lst):
    ...
    if c100 != 'mtp':
        print('Using Brixia')
        val = detectdevice_lst
    else:
        val = i2cdetectdevicelst_mtp
        print('Using for MTP')
...
def Readi2cdeviceviaDiagcommand(mod,i2cdiagcmd):
    ...
    c1=run_command('./cel-i2c-test --read --bus 0 --addr 0x48 --reg16 0x00')
    time.sleep(1)
    if not re.search('0xff',c1) and re.search('I2C read : Passed',c1):
        log.fail('I2c read failed for bus 0')
    for listvalue in i2cdiagcmd:
        cmd_output = run_command('./cel-' + mod + '-test --read --bus ' + listvalue[0] + ' --addr ' + listvalue[1] + ' --reg ' + listvalue[2],prompt=None)
        device.sendCmd('\r')
        time.sleep(5)
...
def checkPsuSensorAndPsuFile():
    ...
    cmd1=run_command("./cel-psu-test --list --file ../configs/psu_B0.yaml")
    time.sleep(3)
    cmd2=run_command("./cel-psu-test --all -f ../configs/psu_B0.yaml")
    if not re.search("POWER test : Passed",cmd2,re.IGNORECASE):
        log.fail("Sensor test failed")
    log.success("sensor test passed")


    cmd3=run_command("./cel-psu-test --all --file ../configs/second_source/psu_B0_flex.yaml")
    time.sleep(3)
    if  re.search("POWER test : FAILED",cmd3,re.IGNORECASE):
        log.success('Power test failed as expected')
    else:
        raise Exception("Power test did not fail as expected")

    cmd4=run_command("./cel-psu-test --all --file ../configs/second_source/psu_B0_astersyn.yaml")
    time.sleep(3)
    if  re.search("POWER test : FAILED",cmd4,re.IGNORECASE):
        log.success('Sensory test failed as expected')
    else:
        raise Exception("Sensory test failed")
...
def checkSoftwareInfo():
    ...
    if c100 != 'mtp':
        ...
        CommonKeywords.should_match_ordered_regexp_list(s3,var.ver_1)
    run_command('exit',prompt='root@sonic')
    run_command('exit')
#####################################################################################################################
#TC 19
...
def checkEepromDump():
    time.sleep(5)
    cmd=run_command('./cel-eeprom-test --dump --dev all --type tlv',prompt=None)
    for i in range(1,9):
        cmd=run_command("./cel-eeprom-test --dump -d {} -t tlv".format(i),prompt=None)
        time.sleep(5)
    eepromDataCheck()

@logThis
def checkEepromRead():
    run_command('./cel-eeprom-test --read --dev all --type tlv',prompt=None)
    for i in range(1,9):
        cmd=run_command("./cel-eeprom-test -r -d {} -t tlv".format(i),prompt=None)
        time.sleep(5)
    eepromDataCheck()
...
@logThis
def checkEepromTest():
    cmd=run_command("./cel-eeprom-test --all",prompt=None,timeout=200)
    eepromDataCheck()

@logThis
def checkEepromData():
    flag = 0 
    cmd=run_command('./cel-eeprom-test --dump --dev all --type tlv',prompt=None,timeout=100)
    device.sendCmd('\r')
    device.sendCmd('\n')
    ...
    #output=run_command(cmd_lst,prompt='root@sonic')
    for x in cmd_lst:
        cmd1=run_command(x,prompt=None)
        device.sendCmd('\r')
        device.sendCmd('\n')
        time.sleep(5)
...
def removeEepromData():
    ...
    for x in cmd_lst:
        ...
        time.sleep(5)
        output.append(cmd1)
    #output=run_command(cmd_lst,prompt='#',timeout=50)
    cmd=run_command('./cel-eeprom-test --dump -d all --type tlv',prompt=None,timeout=60)
...
def writeTlvInfo():
    for i in range (1,3):
        ...
        output=run_command(cmd_lst,prompt=None)
        if "Error" in output:
    ...
    output=run_command(cmd_lst2,prompt=None)
    if "Error" in output:
            raise Exception("cmd failed")
    cmd_lst3=[
    ...
    output=run_command(cmd_lst3,prompt=None)
    if "Error" in output:
            raise Exception("cmd failed")
    for i in range(5,9):
        cmd_lst4=[
        ...
        output=run_command(cmd_lst4,prompt='root@sonic')
        if "Error" in output:
            raise Exception("cmd failed")
    rebootEeprom()
...
def checkAllEepromData():
    cmd= run_command('./cel-eeprom-test -r -d all --type tlv',prompt=None,timeout=100)
...
def programEeprom():
    ...
    output=run_command(cmd_lst,prompt=None,timeout=100)
    rebootEeprom()
################################################################################################################################################
#TC23
@logThis
...
def getMacAdd():
    ...
    if not mat2:
        raise Exception("Failed")
    cmd2=run_command("./eeupdate64e /NIC=3 /MAC_DUMP")
    pat="3: LAN MAC Address is (.+)"
    mat3=re.search(pat,cmd2).group().split()[-1].strip('.')
    if not mat3:
        raise Exception("Failed")
    cmd2=run_command("./eeupdate64e /NIC=4 /MAC_DUMP")
...
@logThis
def checkFWVersion():
    cmd_lst=['./eeupdate64e /bus=11 /dev=0 /fun=0 /eepromver',
             './eeupdate64e /bus=12 /dev=0 /fun=0 /eepromver']
...
@logThis
def upgradei210FW(device):
    cmd_lst=['./eeupdate64e /bus=11 /dev=0 /fun=0 /d /home/Dev_Start_I210_Copper_NOMNG_16Mb_A2_3.25_0.03.bin',
            './eeupdate64e /bus=12 /dev=0 /fun=0 /d /home/Dev_Start_I210_Copper_NOMNG_16Mb_A2_3.25_0.03.bin']
...
@logThis
def downgradei210FW(device):
    cmd_lst=['./eeupdate64e /bus=11 /dev=0 /fun=0 /d /home/Dev_Start_I210_Copper_NOMNG_16Mb_A2_1.0__Rev10.bin',
             './eeupdate64e /bus=12 /dev=0 /fun=0 /d /home/Dev_Start_I210_Copper_NOMNG_16Mb_A2_1.0__Rev10.bin']
    output=run_command(cmd_lst,prompt='#',timeout=150)
    for i in range(0,2):
        if not "Shared Flash image updated successfully." in output:
            raise Exception("Upgrade Failed")
...
@logThis
def  trySsh(a):
    run_command('ssh admin@'+a,prompt='password:',timeout=15)
    device.sendCmd('YourPaSsWoRd')
    device.sendCmd('\r')
...
@logThis
def rebootToSonic():
    device.sendCmd('sudo reboot')
    out=device.read_until_regexp('Shell>',timeout=300)
...
class ParagraphComparer:
    def __init__(self, para1, para2):
        self.para1 = para1
        self.para2 = para2
        self.para1_lines = self.para1.split("\n")
        self.para2_lines = self.para2.split("\n")

    def compare(self):
        if len(self.para1_lines) != len(self.para2_lines):
            return "Paragraphs have different number of lines."

        for i in range(len(self.para1_lines)):
            if self.para1_lines[i] != self.para2_lines[i]:
                return f"Line {i + 1} is different in both paragraphs."

        return "Paragraphs are equal."



@logThis
def rebootEeprom():
    cmd1= run_command('./cel-eeprom-test -r -d all --type tlv',prompt=None,timeout=100)
    time.sleep(30)
    #cmd=run_command('reboot',prompt=None,timeout=300)
    rebootToSonic()
    verifyDiagToolPath()
    time.sleep(100)
    cmd2= run_command('./cel-eeprom-test -r -d all --type tlv',prompt=None,timeout=100)
    time.sleep(30)
    for each in cmd1.splitlines():
        print('Each:',each)
        print('total',cmd2)
        if not re.search(each,cmd2):
            raise Exception("EEprom data isnt matching")
    log.success("EEprom data matched")

def check_me(flag="new"):
    if flag == 'new':
        fr_v1='0xzz'
    else:
        fr_v1='0xa'
    ...
    verifyDiagTool()
    c1=run_command('./lpc_cpld_x64_64 blu r 0xa100',prompt="root@sonic:.*", timeout=5)
    if re.search('0xqa',c1):
        log.success("Baseboard cpld version is correct")
    else:
        raise RuntimeError("Baseboard CPLD version is incorrect")
