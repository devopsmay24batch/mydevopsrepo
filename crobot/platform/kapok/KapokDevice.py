...
import Const
from KapokConst import *


class KapokDevice(Device):

    def __init__(self, deviceDict):
        super().__init__(deviceDict)
        log.debug("Entering KapokDevice class procedure: __init__")
        self.using_pem = -1
        self.promptUboot = deviceDict['promptUboot']
        self.promptUboot1 = deviceDict['promptUboot1']
        self.promptSdk = 'BCM.0>|IVM:0>|IVM>'

    def loginDiagOS(self):
        log.debug("Entering KapokDevice class procedure: loginDiagOS")
        ...
        try:
            self.sendMsg('\n\r')
            self.read_until_regexp(self.promptDiagOS, timeout=5)
            log.info("Already in Diag OS.")
        except:
            try:
                self.read_until_regexp("Please press Enter to activate this console.", timeout=60)
                self.sendCmd('\r', '#', timeout=5)
                #self.getPrompt(self.promptDiagOS)
    ...
    def loginOnie(self):
        log.debug("Entering KapokDevice class procedure: loginOnie")
        self.telnetConnect.open_connection(self.consoleIP, port=self.consolePort,terminal_emulation=True, terminal_type="vt100", window_size="400x100")
        try:
            if self.isBriggs():
                self.getPrompt(Const.ONIE_RESCUE_MODE) # workaround for the sw hang issue of install mode of briggs
            else:
    ...
    def isBriggs(self):
        return 'briggs' in self.name

    def getPrompt(self, mode=None, timeout=60, idleTimeout=60, logFile='None'):
        log.debug('Entering KapokDevice getPrompt with args : %s\n' % (str(locals())))
        output = ''
        outStr = ''
        LogMsg = ''
        self.flush()
    ...
    def bootOnieFromUboot(self, onieBootMode):
        ...
        if 'celestica_cs8200' in out or 'celestica_cs8210' in out or 'celestica_cs8260' in out or 'celestica_cs8264' in out or 'arm64-az9074' in out or 'arm64-az3324-r0' in out:
    ...
    def __rebootToUboot(self):
        ...
        self.read_until_regexp(STOP_AUTOBOOT_PROMPT, BOOT_TIME)
        self.stopAutoBoot()
        self.read_until_regexp(self.promptUboot + '|' + self.promptUboot1)
        self.sendMsg('\r')
        self.currentBootMode = Const.BOOT_MODE_UBOOT
    ...
    def bootDiagOSFromUboot(self):
        ...
        if 'fenghuangv2' in devicename.lower():
            self.read_until_regexp("CEL-DiagOS login:",timeout=60)
            #self.sendCmd('\r','#',timeout=5)
    ...
    def executeCmd(self, cmd, mode=None, timeout=60):
        log.debug('Entering KapokDevice executeCmd with args : %s\n' % (str(locals())))
        if mode != None:
