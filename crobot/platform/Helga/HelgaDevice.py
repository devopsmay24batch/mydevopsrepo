...
import Const
from HelgaConst import *


class HelgaDevice(Device):

    def __init__(self, deviceDict):
        super().__init__(deviceDict)
        log.debug("Entering HelgaDevice class procedure: __init__")
        self.using_pem = -1
        self.promptUboot = deviceDict['promptUboot']
        self.promptSdk = 'BCM.0>|IVM:0>|IVM>'

    def loginDiagOS(self):
        log.debug("Entering HelgaDevice class procedure: loginDiagOS for Helga")
    ...
    def loginOnie(self):
        log.debug("Entering HelgaDevice class procedure: loginOnie")
        self.telnetConnect.open_connection(self.consoleIP, port=self.consolePort,terminal_emulation=True, terminal_type="vt100", window_size="400x100")
        try:
    ...
    def getPrompt(self, mode=None, timeout=60, idleTimeout=60, logFile='None'):
        log.debug('Entering HelgaDevice getPrompt with args : %s\n' % (str(locals())))
        output = ''
        outStr = ''
        LogMsg = ''
        self.flush()
    ...
    def executeCmd(self, cmd, mode=None, timeout=60):
        log.debug('Entering HelgaDevice executeCmd with args : %s\n' % (str(locals())))
        if mode != None:
