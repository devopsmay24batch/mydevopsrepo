...
@logThis
def verifyToolPath():
    output = run_command("cd /root/Diag/aberlour/tools")
    outputPath = run_command("pwd")
    if re.search("/root/Diag/aberlour/tools", outputPath):
...
def cpldListParameter():
    ...
    list_pattern=["Device: sys_cpld, Reg_num: 60, Dev_path: /sys/devices/platform/sys_cpld",
...
def cpldVersionParameter():
    ...
    sys_cpld_version = CommonLib.get_swinfo_dict("ABERLOUR_SYS_CPLD").get("newVersion", "NotFound")
...
@logThis
def sfpToolPath():
    outPut = run_command('cd /root/Diag/aberlour/SDK')
    path = run_command('pwd')
    if re.search("/root/Diag/aberlour/SDK", path):
...
def versionShowParameter():
    ...
    Diag_version =  CommonLib.get_swinfo_dict("ABERLOUR_DIAG").get("newVersion", "NotFound")
    ...
    sdk_version = CommonLib.get_swinfo_dict("ABERLOUR_SDK").get("newVersion", "NotFound")
...
def verifySDKPath():
    output = run_command("cd /root/Diag/aberlour/SDK")
    outputPath = run_command("pwd")
    if re.search("/root/Diag/aberlour/SDK", outputPath):
        log.success("Can find the correct diag tool path")
    else:
...
def warmRebootTest():
    ...
    x= ['cpld','cpu','dcdc','fan','i2c','mem','emmc','pci','psu','pwmon','temp','tpm','usb','eeprom','eeprom','eth','card','version','rtc','python','eeprom','eeprom']
    lst1=[]
...
def marginTest():
    device.sendMsg("cd /root/Diag/aberlour \n\n")
    # At HIGH
...
def upgrade_downgrade_cpu_cpld_version(cpld_bin_file, flag="upgrading"):
    run_command("cd /root/Diag/aberlour")
    old_version = getVersionByCmdAndPattern("./bin/cel-version-test -S", "CPU CPLD Version     :.*", ":")
    output = run_command("./bin/cel-update-test firmware/" + cpld_bin_file)
...
def upgradeEmmcFfu():
    device.sendMsg("cd /root/Diag/aberlour/tools \n\n")
...
def downgrade_Upgrade_System_Cpld_Version(sys_cpld_vme_image, flag="Upgrading"):
    ...
    output = run_command("./bin/cel-vmetools2-test " + sys_cpld_vme_image)
...
#        log.success("iperf tool is present in aberlour device")
#        pass_count +=1
