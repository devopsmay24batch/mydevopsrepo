...
from collections import OrderedDict
import SeastoneOnieVariable as var
from  SEASTONECommonLib import powercycle_device
from SeastoneOnieVariable import *
...
def Switch_ONIE_Mode(mode):
    ...
    out=device.read_until_regexp("Open Network Linux", 120)
    ...
    if mode == 'installer':
        ...
        device.read_until_regexp("GNU GRUB  version 2.04", 100)
        log.debug('entering onie install mode ...')
        bios_menu_lib.send_key(var.device_type, "KEY_ENTER")
        device.read_until_regexp("ONIE: OS Install Mode", 7)
        time.sleep(30)
...
def InstallDiagOS(version, protocol, timeout=600):
    ...
    run_command('root',prompt='Password:')
    run_command('onl')
    if HasError(output):
        log.fail('Have error during ONL Install!')
    log.debug("ONL login is success")
...
def GetONLVersion():
    ...
    res= res.split(" ")
    Img=res[7]
    Img=Img.replace("ONL-DS3000.EVT-","")
...
def Disable_write_protect():
    ...
    device.executeCmd("ipmitest raw 0x3A 0x64 0x00 0x02 0x31 0x03")

@logThis 
def Enable_write_protect():
    #New ONIE release will have the i2c set command for write protect enable and disable
    device.executeCmd("ipmitest raw 0x3A 0x64 0x00 0x02 0x31 0x0F")

@logThis
def Burn_EEPROM():
    device.executeCmd("onie-syseeprom -s 0x21=Seastone2V2")
    device.executeCmd("onie-syseeprom -s 0x22=DHM28-32GDK1KCADF-B039/1")
    device.executeCmd("onie-syseeprom -s 0x23=2CA12301110030007")
    device.executeCmd("onie-syseeprom -s 0x24=B4:DB:91:D0:DF:3B")
    device.executeCmd("onie-syseeprom -s 0x25=”17/07/2023 14:30:00”")
    device.executeCmd("onie-syseeprom -s 0x26=01")
    device.executeCmd("onie-syseeprom -s 0x27=HDS8000")
    device.executeCmd("onie-syseeprom -s 0x28=DS3000")
    device.executeCmd("onie-syseeprom -s 0x29=2023.08")
    device.executeCmd("onie-syseeprom -s 0x2a=6")
    ...
    device.executeCmd("onie-syseeprom -s 0x2e=1.1.4")
...
def StabilityOnieRun():
    ...
    bios_menu_lib.send_key(var.device_type, "KEY_ENTER")
    device.read_until_regexp("ONIE: OS Install Mode", 7)
    output=device.sendCmd("onie-start")
