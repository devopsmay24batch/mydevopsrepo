...
class SEASTONEDevice(Device):

    def __init__(self, deviceDict):
        super().__init__(deviceDict)
        log.debug("Entering MAIN Seastone class procedure: __init__")
        self.using_pem = -1
    ...
    def loginDiagOS(self):
        log.debug("Entering Seastone2v2 class procedure: loginDiagOS")
    ...
    def loginOnie(self):
        log.debug("Entering Seastone class procedure: loginOnie")
        self.telnetConnect.open_connection(self.consoleIP, port=self.consolePort,terminal_emulation=True, terminal_type="vt100", window_size="400x100")
        try:
    ...
    def loginBmc(self):
        log.debug("Entering Seastone class procedure: BMCLOGIN")
    ...
    def getPrompt(self, mode=None, timeout=60, idleTimeout=60, logFile='None'):
        log.debug('Entering Seastone getPrompt with args : %s\n' % (str(locals())))
        output = ''
        outStr = ''
        LogMsg = ''
        self.flush()
    ...
    def __bootIntoOnieMode(self, onieBootMode):
        ...
        if self.currentBootMode == Const.BOOT_MODE_DIAGOS:
            log.debug('Checking current boot mode in seastonev2...')
            self.rebootToOnie(onieBootMode)
        return ''

    def rebootToOnie(self, onieBootMode):
        import bios_menu_lib
        log.debug('Entering procedure seasonev2 rebootToOnie : %s\n' % (str(locals())))
    ...
    def switchToBmc(self):
        log.debug("Entering Seastone Device class procedure: trySwitchToBmc")
        ...
        self.sendMsg("\r\n")
        time.sleep(1)
        log.debug('Logging into the device')
        return ''
    ...
    def loginToNEWBMC(self):
        log.debug("Entering Seastone Device class procedure: loginToNEWBMC")
        self.sendMsg('\r\n')
        self.sendMsg('\r\n')
        time.sleep(10)
        self.sendMsg('\r\n')
        ...
        self.sendMsg('\r\n')
        time.sleep(4)
        self.read_until_regexp(self.promptBmc,timeout=10)
    ...
    def __loginToDiagOS(self):
        log.debug("Entering Seastone Device class procedure:Seastone __loginToDiagOS")
        self.sendMsg(self.rootUserName)
    ...
    def executeCmd(self, cmd, mode=None, timeout=60):
        log.debug('Entering seastone executeCmd with args : %s\n' % (str(locals())))
        if mode != None:
