...
def set_power_cycle(device, lanplus=False, ip_addr=mgmt_ip):
    device_obj = Device.getDeviceObject(device)
    if(lanplus):
        promptServer=device_obj.promptServer
        cmd = "ipmitool -I lanplus -H %s -U root -P 0penBmc -C 17 power cycle \r" % ip_addr
    else:
        cmd = "ipmitool power cycle \r"
    log.info("Turn power cycle through cmd : %s" %(cmd))
    res = device_obj.sendMsg(cmd) 
    log.info('Output : %s' %(res))
...
def update_primary_bmc(device, image):
    ...
    remove_key_cmd='ssh-keygen -f "/home/{}/.ssh/known_hosts" -R "{}"'.format(scp_username, mgmt_ip)
    device_obj.sendCmd(remove_key_cmd, timeout=90)
    ...
    c1 = device_obj.read_until_regexp(r'Are you sure you want to continue connecting (yes/no/[fingerprint])?', timeout=90)
    device_obj.sendCmd("yes", timeout=90)
...
def set_power_chasis(device, status):
    ...
    mgmt_ip = device_obj.managementIP
    promptServer=device_obj.promptServer
    cmd = lanplus_ipmitool_cmd.format("chassis power %s" %status)
    
    log.info("Turn power %s through cmd : %s" %(status, cmd))
    c1=Device.execute_local_cmd(device_obj, cmd)
    log.info('Output : '+c1)
    ...
    if status == 'reset':
        out_3 = device_obj.read_until_regexp(pattern_reset_cause, timeout=400)
        out_3_3 = device_obj.read_until_regexp('localhost login', timeout=400)
        ...
        log.success("reset cause is {} for power reset".format(reset_cause_power_reset))
        device_obj.loginToDiagOS()

    if status == 'cycle':
        out_1 = device_obj.read_until_regexp(pattern_reset_cause, timeout=400)
        out_1_1 = device_obj.read_until_regexp('localhost login', timeout=400)
        x = re.search(pattern_reset_cause, out_1)
        if x is None:
            log.info("reset cause not fetched")
            raise RuntimeError("reset cause not fetched")
        log.info(x.group(1))
        if x.group(1) != reset_cause_power_cycle:
            raise RuntimeError("reset cause is not "+reset_cause_power_cycle)
        log.success("reset cause is {} for power cycle".format(reset_cause_power_cycle))
        device_obj.loginToDiagOS()

    if status == 'on':
        out_2 = device_obj.read_until_regexp(pattern_reset_cause, timeout=400)
        out_2_2 = device_obj.read_until_regexp('localhost login', timeout=400)
        x = re.search(pattern_reset_cause, out_2)
        if x is None:
            log.info("reset cause not fetched")
            raise RuntimeError("reset cause not fetched")
        log.info(x.group(1))
        if x.group(1) != reset_cause_power_on:
            raise RuntimeError("reset cause is not "+reset_cause_power_on)
        log.success("reset cause is {} for power on".format(reset_cause_power_on))
        device_obj.loginToDiagOS()

def start_ping_bmc_ip(device, bmc_ip):
    device_obj = Device.getDeviceObject(device)
    mgmt_ip = device_obj.managementIP 
    promptServer=device_obj.promptServer
    log.info("Sleeping for 5 seconds.")
    time.sleep(5)
    cmd='ping -c 120 {} >ping_output.log 2>&1 &'.format(bmc_ip)
    log.info("Starting ping to bmc ip in background through cmd : %s" %cmd)
    c1=Device.execute_local_cmd(device_obj, cmd)

 
def check_ping_bmc_ip(device):
    device_obj = Device.getDeviceObject(device)
    mgmt_ip = device_obj.managementIP 
    promptServer=device_obj.promptServer
    log.info("Sleeping for 30 seconds.")
    time.sleep(30)
    cmd='cat ping_output.log'
    log.info("Checking ping output through cmd : %s" %cmd)
    c1=Device.execute_local_cmd(device_obj, cmd)
    log.info(c1)
    
    ping_pattern = " 0% packet loss"
    if ping_pattern in c1:
        log.info("bmc communication is as expected!!")
    else:
        raise RuntimeError("bmc communication is not as expected")

    cmd='rm ping_output.log'
    log.info("Remove ping output file : %s" %cmd)
    c1=Device.execute_local_cmd(device_obj, cmd)
...
def verify_dependent_image_using_diag():
    ...
    pattern.append("OS option .*: {}".format(onl_version))
...
def check_kcs_interface_stress_test(device):
    ...
    device_obj = Device.getDeviceObject(device)
    duration = kcs_interface_stress_test_duration
    promptServer=device_obj.promptServer
    ...
    timeout_stress = duration + 300
    out=device_obj.executeCmd(cmd, timeout=timeout_stress)
