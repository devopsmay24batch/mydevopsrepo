...
mgmt_ip = DeviceMgr.getDevice(devicename).get('bmcIP')
mgmt_ip_onl = DeviceMgr.getDevice(devicename).get('managementIP')
...
set_bmc_ipaddr = DeviceMgr.getDevice(devicename).get('bmcIP_Static_1')
set_bmc_ipaddr_2 = DeviceMgr.getDevice(devicename).get('bmcIP_Static_1')
dhcp_add = 'DHCP Address'
set_bmc_netmask = DeviceMgr.getDevice(devicename).get('bmcSubnetMask')
...
sel_list_after_clear_pattern= "SEL has no entries"
...
kcs_interface_stress_test_duration=43200

pattern_reset_cause='Reset cause(\S+):'
reset_cause_reboot='(0x11)'
reset_cause_power_reset='(0x22)'
reset_cause_power_cycle='(0x77)'
reset_cause_power_on='(0x11)'
